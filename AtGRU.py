...
class AtGRU():
    ...
    def run(self):
        ...
        metrics0_.to_excel(r"AtGRU评价数据\AtGRU初预测效果评价1.xlsx")
        #
        #计算第二次预测评价指标
        ...
        metrics1_.to_excel(r"AtGRU评价数据\AtGRU初预测效果评价2.xlsx")
        #绘制结果图1
        plt.figure(1)
        ...
        plt.legend()
        plt.savefig(r"AtGRU评价数据\AtGRU预测图1")
        plt.show()
        #绘制结果图2
        plt.figure(2)
        ...
        plt.ylabel("Wind speed value")
        plt.legend()
        plt.savefig(r"AtGRU评价数据\AtGRU预测图2")
        plt.show()
        #保存数据与模型
        total_error1 =np.append(pred_-test_,pred_2-test_2)
        np.savetxt('AtGRU预测数据\AtGRU Preliminary label.csv',test_,delimiter=',')
        np.savetxt('AtGRU预测数据\AtGRU Preliminary forecast.csv', pred_, delimiter=',')
        np.savetxt('AtGRU预测数据\AtGRU Predict_error.csv',pred_-test_,delimiter=',')
        np.savetxt('AtGRU预测数据\AtGRU Final forecast.csv', pred_2, delimiter=',')
        np.savetxt('AtGRU预测数据\AtGRU Fianl_error.csv',pred_2-test_2,delimiter=',')
        np.savetxt('AtGRU预测数据\AtGRU Final label.csv', test_2, delimiter=',')
        np.savetxt('AtGRU预测数据\TotalError.csv',total_error1,delimiter=',')
    ...
    class ErrorProcessed():
        ...
        def run2(self):
            ...
            for n in range(0, 10):
                ...
                # 搭建预测模型
                model = keras.models.Sequential()
                model.add(keras.layers.GRU(64, input_shape=(X_train.shape[1], 1), return_sequences=True))
                model.add(keras.layers.Dropout(0.1))
                model.add(attention.Attention())
        ...
        def run3(self,error_sequence):
            '''5.最后评价'''
            #读入评测的标签数据以及预测数据
            label = pd.read_csv(f'{self.name}\AtGRU Final label.csv',header=None)
            pre = pd.read_csv(f'{self.name}\AtGRU Final forecast.csv',header=None)
            # dataset = datasets.iloc[:,[5]].values
            ...
            metrics2.to_excel(f"{self.name_}\ECAtGRU预测效果评价.xlsx")
            ...
            plt.ylabel("Wind speed value")
            plt.legend()
            plt.savefig(f"{self.name_}\ECAtGRU预测图")
            plt.show()
            print("处理完毕！")