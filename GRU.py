...
class GRU():
    ...
    def run(self):
        ...
        model.add(tf.keras.layers.GRU(64, input_shape=(self.X_train.shape[1],1)))
        ...
        metrics0_.to_excel(r"GRU评价数据\GRU初预测效果评价1.xlsx")
        #
        # 计算第二次预测评价指标
        ...
        metrics1_.to_excel(r"GRU评价数据\GRU初预测效果评价2.xlsx")
        # 绘制结果图1
        plt.figure(1)
        ...
        plt.ylabel("Wind speed value")
        plt.legend()
        plt.savefig(r"GRU评价数据\GRU预测图2")
        plt.show()
        # 保存数据与模型
        total_error1 = np.append(Y_pre - Y_test, pred_2 - test_2)
        np.savetxt('GRU预测数据\GRU Preliminary label.csv', Y_test, delimiter=',')
        np.savetxt('GRU预测数据\GRU Preliminary forecast.csv', Y_pre, delimiter=',')
        np.savetxt('GRU预测数据\GRU Predict_error.csv', Y_pre - Y_test, delimiter=',')
        np.savetxt('GRU预测数据\GRU Final forecast.csv', pred_2, delimiter=',')
        np.savetxt('GRU预测数据\GRU Fianl_error.csv', pred_2 - test_2, delimiter=',')
        np.savetxt('GRU预测数据\GRU Final label.csv', test_2, delimiter=',')
        np.savetxt('GRU预测数据\TotalError.csv', total_error1, delimiter=',')
        # 搭建误差序列分解类(内部类)
    ...
    class ErrorProcessed():
        ...
        def run3(self, error_sequence):
            '''5.最后评价'''
            # 读入评测的标签数据以及预测数据
            label = pd.read_csv(f'{self.name}\GRU Final label.csv', header=None)
            pre = pd.read_csv(f'{self.name}\GRU Final forecast.csv', header=None)
            # dataset = datasets.iloc[:,[5]].values
            ...
            metrics2.to_excel(f"{self.name_}\ECGRU预测效果评价.xlsx")
            ...
            plt.ylabel("Wind speed value")
            plt.legend()
            plt.savefig(f"{self.name_}\ECGRU预测图")
            plt.show()
