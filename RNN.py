...
class RNN():
    ...
    def run(self):
        ...
        metrics0_.to_excel(r"RNN评价数据\RNN初预测效果评价1.xlsx")
        #
        # 计算第二次预测评价指标
        ...
        metrics1_.to_excel(r"RNN评价数据\RNN初预测效果评价2.xlsx")
        # 绘制结果图1
        plt.figure(1)
        ...
        plt.ylabel("Wind speed value")
        plt.legend()
        plt.savefig(r"RNN评价数据\RNN预测图2")
        plt.show()
        # 保存数据与模型
        total_error1 = np.append(Y_pre - Y_test, pred_2 - test_2)
        np.savetxt('RNN预测数据\RNN Preliminary label.csv', Y_test, delimiter=',')
        np.savetxt('RNN预测数据\RNN Preliminary forecast.csv', Y_pre, delimiter=',')
        np.savetxt('RNN预测数据\RNN Predict_error.csv', Y_pre - Y_test, delimiter=',')
        np.savetxt('RNN预测数据\RNN Final forecast.csv', pred_2, delimiter=',')
        np.savetxt('RNN预测数据\RNN Fianl_error.csv', pred_2 - test_2, delimiter=',')
        np.savetxt('RNN预测数据\RNN Final label.csv', test_2, delimiter=',')
        np.savetxt('RNN预测数据\TotalError.csv', total_error1, delimiter=',')
    # 搭建误差序列分解类(内部类)
    ...
    class ErrorProcessed():
        ...
        def run3(self, error_sequence):
            '''5.最后评价'''
            # 读入评测的标签数据以及预测数据
            label = pd.read_csv(f'{self.name}\RNN Final label.csv', header=None)
            pre = pd.read_csv(f'{self.name}\RNN Final forecast.csv', header=None)
            # dataset = datasets.iloc[:,[5]].values
            ...
            metrics2.to_excel(f"{self.name_}\ECRNN预测效果评价.xlsx")
            ...
            plt.ylabel("Wind speed value")
            plt.legend()
            plt.savefig(f"{self.name_}\ECRNN预测图")
            plt.show()
