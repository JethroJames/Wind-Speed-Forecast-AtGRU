...
class BPNN():
    ...
    def run(self):
        ...
        # model.fit()对模型进行训练
        model.fit(self.X_train, self.Y_train, batch_size=64, epochs=150)
        model.summary()
        ...
        metrics0_.to_excel(r"BPNN评价数据\BPNN 初预测效果评价1.xlsx")
        #
        # 计算第二次预测评价指标
        ...
        metrics1_.to_excel(r"BPNN评价数据\BPNN 初预测效果评价2.xlsx")
        # 绘制结果图1
        plt.figure(1)
        ...
        plt.legend()
        plt.savefig(r"BPNN评价数据\BPNN 预测图1")
        plt.show()
        # 绘制结果图2
        plt.figure(2)
        ...
        plt.ylabel("Wind speed value")
        plt.legend()
        plt.savefig(r"BPNN评价数据\BPNN 预测图2")
        plt.show()
        # 保存数据与模型
        total_error2 = np.append(Y_pre - Y_test, pred_2 - test_2)
        np.savetxt('BPNN预测数据\BPNN Preliminary label.csv', Y_test, delimiter=',')
        np.savetxt('BPNN预测数据\BPNN Preliminary forecast.csv', Y_pre, delimiter=',')
        np.savetxt('BPNN预测数据\BPNN Predict_error.csv', Y_pre - Y_test, delimiter=',')
        np.savetxt('BPNN预测数据\BPNN Final forecast.csv', pred_2, delimiter=',')
        np.savetxt('BPNN预测数据\BPNN Fianl_error.csv', pred_2 - test_2, delimiter=',')
        np.savetxt('BPNN预测数据\BPNN Final label.csv', test_2, delimiter=',')
        np.savetxt('BPNN预测数据\TotalError.csv', total_error2, delimiter=',')
    ...
    class ErrorProcessed():
        ...
        def run3(self, error_sequence):
            '''5.最后评价'''
            # 读入评测的标签数据以及预测数据
            label = pd.read_csv(f'{self.name}\BPNN Final label.csv', header=None)
            pre = pd.read_csv(f'{self.name}\BPNN Final forecast.csv', header=None)
            # dataset = datasets.iloc[:,[5]].values
            ...
            metrics2.to_excel(f"{self.name_}\ECBPNN预测效果评价.xlsx")
            ...
            plt.ylabel("Wind speed value")
            plt.legend()
            plt.savefig(f"{self.name_}\ECBPNN预测图")
            plt.show()
